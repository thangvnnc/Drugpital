...
router.get("/get/type/find", (req, res) => {
    ...
    // Lấy số lượng phần tử
    MySqlCon.query("SELECT COUNT(`id`) as total FROM `type_medicine` WHERE `description` LIKE ?", [query], function (error, results) {
        if (error) {
            res.send(Error.ERR_EXECUTE_DB());
            Log.write(error);
            return;
        ...
        // Lấy dữ liệu danh sách thuốc
        MySqlCon.query("SELECT * FROM `type_medicine` WHERE `description` LIKE ? LIMIT ?, ?", [query, startRecord, record],
            function (error, results) {
                if (error) {
                    res.send(Error.ERR_EXECUTE_DB());
                    Log.write(error);
