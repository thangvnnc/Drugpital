...
router.get("/get", (req, res) => {
    ...
    MySqlCon.query('SELECT COUNT(medicine_id) as total FROM `medicine`', function (error, results) {
        if (error) {
            res.send(Error.ERR_EXECUTE_DB());
            Log.write(error);
            return;
        ...
        // Lấy dữ liệu danh sách thuốc
        MySqlCon.query("SELECT * FROM `medicine` LIMIT ?, ?", [startRecord, record], function (error, results) {
            if (error) {
                res.send(Error.ERR_EXECUTE_DB());
                Log.write(error);
                return;
...
router.get("/find", (req, res) => {
    ...
    // Lấy số lượng phần tử
    MySqlCon.query("SELECT COUNT(`medicine_id`) as total FROM `medicine` WHERE `description` LIKE ?", [query], function (error, results) {
        if (error) {
            res.send(Error.ERR_EXECUTE_DB());
            Log.write(error);
            return;
        ...
        // Lấy dữ liệu danh sách thuốc
        MySqlCon.query("SELECT * FROM `medicine` WHERE `description` LIKE ? LIMIT ?, ?", [query, startRecord, record],
...
router.get("/auto", (req, res) => {
    ...
    MySqlCon.query("SELECT `medicine_id`, `description` FROM `medicine` WHERE `description` LIKE ? LIMIT 0, 5", [query],
        function (error, results) {
            if (error) {
                res.send(Error.ERR_EXECUTE_DB());
                Log.write(error);
